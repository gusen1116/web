...
Utility routes for port scanning and DNS lookups. The DNS lookup route
now declares the domain name as a route parameter instead of relying on
trailing whitespace. Additional comments explain the CSRF validation logic.
"""
...
def is_valid_host(host: str) -> bool:
    """Return True if the given host is a valid hostname or IP address."""
    try:
...
def start_portscan():
    """Begin a port scan and stream results back to the client.

    CSRF validation is performed either via headers or form data depending on
    the content type. JSON requests must provide the token in headers.
    """
    try:
        ...
        if csrf_token:
            ...
            if not csrf_token:
                return Response(json.dumps({'error': 'CSRF 토큰이 필요합니다'}), status=403, mimetype='application/json')
    except Exception:
        return Response(json.dumps({'error': 'CSRF 검증 실패'}), status=403, mimetype='application/json')
    if request.is_json:
        ...
        if isinstance(ports_to_scan, str):
            try:
                ...
                return Response(json.dumps({'error': '유효하지 않은 포트 형식입니다.'}), status=400, mimetype='application/json')
    else:
    ...
    if not isinstance(ports_to_scan, list) or not all(isinstance(p, int) for p in ports_to_scan):
        return Response(json.dumps({'error': '포트 목록은 정수형 배열이어야 합니다.'}), status=400, mimetype='application/json')
    try:
...
def dns_lookup(domain_name: str):
    """Perform a DNS lookup for the given domain and return its IP addresses."""
    ...
    try:
        ...
        ip_addresses = sorted(list({res[4][0] for res in results}))
        if not ip_addresses:
            return jsonify({'error': f"'{domain_name}'에 대한 IP 주소를 찾을 수 없습니다."}), 404
        return jsonify({'domain': domain_name, 'ip_addresses': ip_addresses})
    except socket.gaierror:
...
def api_port_scan():
    """Perform a simple port scan via the API (non-streaming)."""
