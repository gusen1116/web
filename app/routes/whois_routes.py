...
# PywhoisError 임포트 오류 해결을 위한 안전한 임포트 방식
try:
    from whois.parser import PywhoisError
except ImportError:
    try:
        # 일부 버전에서는 whois.whois 내부에 위치함
        from whois.whois import PywhoisError
    except ImportError:
        # 둘 다 없을 경우 일반 Exception을 사용하도록 대체
...
@whois_bp.route('/')
def whois_page():
    """Render the WHOIS lookup page."""
    return render_template('whois.html')

@whois_bp.route('/api/<domain_name>')
