...
from flask import Flask, g, request, render_template # render_template 추가 (에러 핸들러에서 사용)
from flask_compress import Compress
from flask_talisman import Talisman
from flask_wtf.csrf import CSRFProtect # CSRFProtect 임포트
...
compress = Compress()
talisman = Talisman()
csrf = CSRFProtect() # CSRFProtect 인스턴스 생성
...
def create_app(config_name=None):
    ...
    csrf.init_app(app) # CSRFProtect 앱에 초기화
...
def register_error_handlers(app):
    ...
    def not_found_error(error):
        ...
        return render_template('404.html'), 404 # render_template 임포트 필요
    
    @app.errorhandler(500)
    def internal_error(error):
        app.logger.error(f'500 에러: {error}')
        return render_template('500.html'), 500 # render_template 임포트 필요
    
    @app.errorhandler(403)
    def forbidden_error(error):
        app.logger.warning(f'403 에러: {request.url}')
        return render_template('403.html'), 403 # render_template 임포트 필요
